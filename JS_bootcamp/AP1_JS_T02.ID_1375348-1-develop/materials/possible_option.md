# Possible implementation of the game "Sea Confrontation"
...
class Board {
  ...
  receiveAttack(x, y) {
    ...
    if (target instanceof Ship) {
      const hitPosition = target.isVertical
        ? x - target.startPosition.x
        : y - target.startPosition.y;
    ...
    for (let row = 0; row < this.size; row++) {
      ...
      for (let col = 0; col < this.size; col++) {
        if (
          ...
          ((this.grid[row][col].isVertical &&
            this.grid[row][col].hits[row - this.grid[row][col].x]) ||
            (!this.grid[row][col].isVertical &&
              this.grid[row][col].hits[col - this.grid[row][col].y]))
        ) {
          rowString += "X ";
...
class HumanPlayer extends Player {
  ...
  takeTurn(opponent) {
    const playerInput = prompt(
      `${this.name}, enter attack coordinates (e.g., "A3"): `
    );
...
class App {
    ...
    if (player instanceof HumanPlayer) {
      for (let i = 0; i < shipCount; i++) {
        const shipInput = prompt(`
            ${player.name}, enter ship length (up to ${maxShipLength}), starting coordinate (e.g., A 1),
      ...
      const startPos = {
        x: Math.floor(Math.random() * maxShipLength),
        y: Math.floor(Math.random() * maxShipLength),
      };
  ...
  run() {
    const isHumanPrompt = prompt(`Play against a human? (1 - yes, 0 - no)`);
    ...
    while (true) {
      console.log(`Board of ${this.firstPlayer.name}:`);

      this.firstPlayer.board.display();

      console.log(`\nBoard of ${this.secondPlayer.name}:`);

      this.secondPlayer.board.display();

      const { x, y, opponent } = currentPlayer.takeTurn(
      ...
      if (successfulAttack) {
        console.log(`${currentPlayer.name}, successful attack!`);

        const isSunk = opponent.board.ships.every((ship) => ship.isSunk());

        if (isSunk) {
          console.log(
            `${currentPlayer.name} has sunk all enemy ships! Congratulations, you win!`
          );
